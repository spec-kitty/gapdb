...
- 2026-08-23T20:35:47Z – implementer-ivan – shell_pid=0 – WP06 focused repair 64391d0. Red: ok  	gapdb/internal/server	0.012s failed both probes: flocked replacement reached readiness; watch returned ambiguous TransportError instead of REVISION_AHEAD. Green: exact pair x10 normal/race; real gapdbd contract; all 41 error schemas can terminally end pre-start watch; full uncached normal/race; vet/staticcheck/govulncheck; mod verify/tidy; gofmt/diff; storage fuzz 5s. Runtime lease now retains acquisition directory fd, fchmod/fstats actual held descriptor and anchored fstatat LOCK identity immediately before listen. Registration errors use ok:false stream:ended reason:error and Client.Watch returns typed remote errors (ahead/compacted/busy covered). Governance defects: charter checklist fetch failed ; pre-review gate no_coverage missing tests.architectural._gate_coverage; protected-main + exact 154 governance-commit ancestry guard required documented --force.
- 2026-08-23T20:35:58Z – implementer-ivan – shell_pid=0 – Correction to prior WP06 history note: the exact charter command was spec-kitty charter context --include section:code-review-checklist and it failed with: Error: No charter section found for selector 'section:code-review-checklist'. No terminology drift occurred. The prior shell printed '/bin/bash: line 1: No: command not found' because the quoted note contained command-markup characters; add-history itself still succeeded. Ruff diff-scoped check found no changed Python files and exited 0.
- 2026-08-23T20:50:30Z – implementer-ivan – shell_pid=0 – WP06 directory-authority repair f526113. Red: go test ./internal/server -run ^TestReviewerDatabaseDirectorySwapFailsBeforeReadiness$ -count=1 returned replacement database directory received readiness. Green: three boundary matrix (before first check, after stale cleanup, after listener bind) x10 normal/race; plausible replacement LOCK/socket/sentinel untouched and anchored socket cleaned; real gapdbd normal/race; full uncached normal/race; vet/staticcheck/govulncheck; mod verify/tidy; gofmt/diff; no Python in ruff diff; fuzz snapshot 986472 and storage 93422. Startup retains database parent/name identity, fchmods directory FD, and uses anchored socket-parent descriptors for stale unlink, procfs bind, socket chmod/identity verification, and cleanup. Linux procfs requirement documented in x-sys evidence. Protected-main and exact 162 governance-commit ancestry guards required documented force; pre-review no_coverage still lacks tests.architectural._gate_coverage.
